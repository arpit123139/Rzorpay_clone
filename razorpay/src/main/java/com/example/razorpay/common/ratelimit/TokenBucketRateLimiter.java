...
public class TokenBucketRateLimiter implements RateLimiter {
    ...
    @Override
    public RateLimitingResult check(String key, int maxRequests, long windowSeconds) {
        try {
            ...
            return allowed ? RateLimitingResult.allowed(remaining) : RateLimitingResult.denied(Math.max(1, retryAfter));
        } catch (DataAccessException e) {
            log.warn("Rate limiter unavailable, failing open for key={}", key, e);
            return RateLimitingResult.allowed(maxRequests);
        }
    }
}
